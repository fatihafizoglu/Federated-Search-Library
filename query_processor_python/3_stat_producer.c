...
#define QUERY_FILE_PATH "queries_MAIN.txt"
#define SELECTED_RESOURCES_FILE_PATH "selected_resources_TOPIC_CSI.txt"
...
void main(void) {
    ...
    // unsigned int **posting_list_access_statistics_sum;
    ...
    // posting_list_access_statistics_sum = (unsigned int**) malloc(NO_OF_QUERY * sizeof(unsigned int*));
    // for ( query_index = 0; query_index < NO_OF_QUERY; query_index++) {
    //     posting_list_access_statistics_sum[query_index] = (unsigned int*) malloc(NO_OF_METHOD * sizeof(unsigned int));
    //     for ( i = 0; i < NO_OF_METHOD; i++) {
    //         posting_list_access_statistics_sum[query_index][i] = 0;
    //     }
    // }
    ...
    for ( query_index = 0; query_index < NO_OF_QUERY; query_index++) {
        ...
        cluster_accesses = (unsigned int**) malloc(NO_OF_METHOD * sizeof(unsigned int*));
        for ( i = 0; i < NO_OF_METHOD; i++) {
            cluster_accesses[i] = (unsigned int*) malloc(NO_OF_CLUSTERS * sizeof(unsigned int));
            for ( j = 0; j < NO_OF_CLUSTERS; j++) {
                cluster_accesses[i][j] = 0;
            }
        }
        for ( i = 0; i < DOC_NO; i++) {
            cluster_accesses[0][0] += document_accesses[i]; // EXHAUSTIVE - Increment first cluster
            ...
            for ( j = 0; j < selected_resources_lengths[query_index]; j++) {
                if (selected_resources[query_index][j] == topic_cluster_id) {
                    cluster_accesses[1][topic_cluster_id] += document_accesses[i]; // TOPIC - Increment documents topic cluster
                    cluster_accesses[2][random_cluster_id] += document_accesses[i]; // RANDOM AND TOPIC - Increment documents topic cluster
                    break;
                }
            }

            cluster_accesses[3][random_cluster_id] += document_accesses[i]; // RANDOM - Increment documents random cluster
        }

        for ( i = 0; i < NO_OF_METHOD; i++) {
            total_posting_list_checked = 0;
            max_posting_list_checked = 0;
            for ( j = 0; j < NO_OF_CLUSTERS; j++) {
                total_posting_list_checked += cluster_accesses[i][j];
                if (cluster_accesses[i][j] > max_posting_list_checked) {
                    max_posting_list_checked = cluster_accesses[i][j];
                }
            }
            printf("%u\n", max_posting_list_checked);
            printf("%u\n", total_posting_list_checked);
            printf("-------\n");
        }
        printf("************************************************************************************\n");
        ...
        for ( i = 0; i < NO_OF_METHOD; i++) {
            free(cluster_accesses[i]);
        }
        free(cluster_accesses);
